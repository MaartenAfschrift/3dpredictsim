...
constexpr int NU = ndof + 6;           // # controls
...
int F_generic(const T** arg, T** res) {
	...
	/// apply the exoskeleton torque - Ankle
	Vec3 ExoAnkle_l = Vec3(0);
	Vec3 ExoAnkle_r = Vec3(0);
	ExoAnkle_l[2] = u[ndof];
	ExoAnkle_r[2] = u[ndof + 1];
	ExoActuation(*model, *tibia_l, *calcn_l, *state, ExoAnkle_l, appliedBodyForces);
	ExoActuation(*model, *tibia_r, *calcn_r, *state, ExoAnkle_r, appliedBodyForces);

	/// apply the exoskeleton torque - Knee
	Vec3 ExoKnee_l = Vec3(0);
	Vec3 ExoKnee_r = Vec3(0);
	ExoKnee_l[2] = u[ndof + 2];
	ExoKnee_r[2] = u[ndof + 3];
	ExoActuation(*model, *femur_l, *tibia_l, *state, ExoKnee_l, appliedBodyForces);
	ExoActuation(*model, *femur_r, *tibia_r, *state, ExoKnee_r, appliedBodyForces);

	/// apply the exoskeleton torque - Hip
	Vec3 ExoHip_l = Vec3(0);
	Vec3 ExoHip_r = Vec3(0);
	ExoHip_l[2] = u[ndof + 4];
	ExoHip_r[2] = u[ndof + 5];
	ExoActuation(*model, *pelvis, *femur_l, *state, ExoHip_l, appliedBodyForces);
	ExoActuation(*model, *pelvis, *femur_r, *state, ExoHip_r, appliedBodyForces);
